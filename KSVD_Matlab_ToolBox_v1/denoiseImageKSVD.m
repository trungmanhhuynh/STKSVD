...
slidingDis = 2;                         %sliding distance for bounding boxs 
bb = 8;                                 %sizes of bounding box
maxNumBlocksToTrainOn = 65000;
displayFlag = 1;

param.K = K;                            %size of dictionary 
...
DCT=kron(DCT,DCT);
%}
[blkMatrix,idx] = my_im2col(Image,[bb,bb],slidingDis);
DCT = blkMatrix ;
...
for jj = 1:30000:size(blocks,2)
    ...
    Coefs = OMP(Dictionary,blocks(:,jj:jumpSize), param.L);
    if (reduceDC)
