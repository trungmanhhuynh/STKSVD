...
function [Trk, Obs_grap, Obs_info] = MOT_Local_Association(Trk, detections, Obs_grap, param, fr, rgbimg, KSVDparam, LCKSVDparam)
...
if ~isempty(ystate)
    ...
    if ~isempty(tidx) && ~isempty(yidx)
        ...
        stage = 1; 
        [score_mat] = mot_eval_association_matrix(stage,tidx, Trk_high, Z_set, param,'Obs', KSVDparam,LCKSVDparam,1);
        ...
        if ~isempty(matching)
            for i=1:size(matching,1)
                ...
                if(stage == 1) 
                    ...
                    Trk(ta_idx).hyp.score(fr) = 0.9 ;       % reset confident level
                end 
