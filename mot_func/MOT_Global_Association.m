...
if ~isempty(low_indx)
    ...
    stage = 2; 
    [score_trk] = mot_eval_association_matrix(stage,low_indx,Low_trk,High_trk,param,'Trk',KSVDparam,LCKSVDparam,1);
    [score_obs] = mot_eval_association_matrix(stage,low_indx,Low_trk, Y_set, param,'Obs', KSVDparam,LCKSVDparam,1);
        ...
        if mat_idx <= length(high_indx)
            ...
            if ~isOverlap
                 if(stage == 1) 
                    ...
                    Trk(t_idx).hyp.score(fr) = 0.9 ;       % reset confident level
                    %Trk(t_idx).hyp.score(Trk(t_idx).ifr:fr) = 0.9 ;
                end 
