...
for i=1:length(Y_set(fr).child)
    ...
    if  length(find(ass_idx ~=0)) >= new_thr
        ...
        % check if one of detection in ass_idx overlaps with
        % high confident existing track then that detection is unreliable
        % and should be remove from ass_idx
        
        isOverlap = check_tracklet_overlap(ass_idx, detections, Trk, fr ,new_thr)    ;
        if(~isOverlap)
            ct = ct + 1;
...
function isOverlap = check_tracklet_overlap(ass_idx, detections, Trk, fr ,new_thr)
isOverlap = false ;
for trackId = 1: length(Trk)
   % if(Trk(trackId).last_update == fr)
        for tmpid = fr:-1:fr-new_thr+1
            trackstate = Trk(trackId).state{fr}' ;
            detState  = [detections(tmpid).x(ass_idx(tmpid)),detections(tmpid).y(ass_idx(tmpid)), ...
                detections(tmpid).w(ass_idx(tmpid)),detections(tmpid).h(ass_idx(tmpid))];
            overlapRatio  =   bboxOverlapRatio(trackstate,detState) ;
            if(overlapRatio > 0.3)
                isOverlap = true ;
                break;
            end
