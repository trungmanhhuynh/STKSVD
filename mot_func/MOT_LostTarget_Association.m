function [Trk, Obs_grap] = MOT_LostTarget_Association(Trk, Obs_grap, Obs_info, param, KSVDparam, LCKSVDparam,fr)
%% Copyright (C) 2014 Seung-Hwan Bae
%% All rights reserved.

Refer =[]; Test= [];
all_indx = 1:length(Trk);
[lost_indx] = Idx2Types(Trk,'Lost');
[low_indx] = Idx2Types(Trk,'Low');
[high_indx] = setdiff(all_indx, [lost_indx low_indx] ) ;
...
if ~isempty(lost_indx)
    % For tracklets with low confidence
    for ii=1:length(lost_indx)
        i = lost_indx(ii);
    ...
    thr = param.global_thr;
    
    [score_trk] = mot_eval_association_matrix(lost_indx,Low_trk,High_trk,param,'Trk',KSVDparam,LCKSVDparam,0);
    [score_obs] = mot_eval_association_matrix(lost_indx,Low_trk, Y_set, param,'Obs', KSVDparam,LCKSVDparam,0);
    score_mat = score_obs ; %[score_trk, score_obs];
    ...
    for m=1: length(Affinity)
        ...
        if mat_idx <= length(high_indx)
            ...
            %not assign observation with low tracklets
            %when this observation overlaps with other high confident track
            isOverlap = check_tracklet_overlap(ystate(:,y_idx), Trk,fr);
            if ~isOverlap
...
%%
function isOverlap = check_tracklet_overlap(ystate, Trk,fr)
isOverlap = false ;
for trackId = 1: length(Trk)
    if(Trk(trackId).last_update == fr)
        
        trackstate = Trk(trackId).state{fr-1}' ;
        overlapRatio  =   bboxOverlapRatio(trackstate,ystate','Min') ;
        if(overlapRatio > 0.3)
            isOverlap = true ;
            break;
        end
